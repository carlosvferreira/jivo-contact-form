...
class App extends Component {
  ...
  formNameFormatter = () => {
    ...
    // Old version of this code below for info - was changing the state without setState, which is bad
    // this.state.formControls.name.value = userNameFormatted;
    // return this.state.formControls.name.value;
  };

  //Real-time auto-formatter for emails inserted in the form

  formEmailFormatter = () => {
    if (
      this.state.formControls.email &&
      this.state.formControls.email.value.includes("@")
    ) {
  ...
  formSendDataToChat = () => {
    window.jivo_api.setContactInfo({
      ...
      description: `${
        this.state.formControls.name.value.split(" ")[0]
      } has selected the ${
        this.state.formControls.department.value
      } department`,
    });
  };
  ...
  formSendProactiveInvitation = () => {
    ...
    window.jivo_api.showProactiveInvitation(
      "Hello, dear " + clientFirstName + "! How can I help you today?"
    );
  };
  ...
  async componentDidMount() {
    try {
      const status = await fetch(
        "//node136.jivosite.com/widget/status/497422/ZLM6UIaTWf"
      );
  ...
  render() {
    return (
      <div className="App">
        ...
        {!this.state.chatStatus && !this.state.isLoading && (
          <p id="offlineChat">
            We're currently offline. Please try again later.
          </p>
        )}
        {this.state.loadingError && (
          <p id="failMessage">Failed to load, please refresh to try again.</p>
        )}

        <form
          id="chatForm"
          disabled={this.state.isLoading || !this.state.chatStatus}
        >
          <Name
            name="name"
            disabled={this.state.isLoading || !this.state.chatStatus}
          ...
          {!this.state.formControls.name.valid &&
            this.state.formControls.name.touched && (
              <p className="invalid-field-message">
          ...
          {!this.state.formControls.department.valid &&
            this.state.formControls.department.touched && (
              <p className="invalid-field-message">
